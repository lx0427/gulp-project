...
function queryData() {
    ...
    $.ajaxjsonp(url, dataMap, function(data) {
        ...
        // 是否有融资
        if((isNull(d.creditFlag)||d.creditFlag==0) && (isNull(d.XfcreditFlag)||d.XfcreditFlag==0)){
            $(".wx_main").css('bottom','98px');
        }
        if(d.creditFlag==1){
        ...
        $("#footer").html(template('footer_page', d));
        if(d!=null && d.data!=null && d.data.length>0){
            $("#balance").attr('onclick','toConfirmOrder()');
        ...
        if(d!=null && d.menber!=null){
            $(".mbname").html(d.menber.mbname);
        }
        ...
        // 是否融资
        $(".wx_isrz a").bind("click", function() {
            $(".wx_isrz a").removeClass("current");
            $(this).addClass("current");
            totalAmt();
        });
        // 是否有优惠券
...
function doCheck(pkid) {
    if ($("#" + pkid).hasClass("current")) {
        ...
        $("#allCheckicon").removeClass("current");
    } else {
        ...
        if(checkFlag){
            $("#allCheckicon").addClass("current");
        }
    }
    
    var cleckLis = $("ul.current");
		var liBlock = true;
		for (var i=0;i<cleckLis.length;i++) {
			var liId = $(cleckLis[i]).attr("checkid");
    ...
    var b = true;
    var oldCfxx = "";
    $(".checkicon .current").each(function() {       
        var id = $(this).attr("id");
...
//防止下预约订单的时候,里面既有价格的明细又有没价格的明细,这里限定只能统统都是没有价格的明细才让下预约订单
var hasPrice1 = false; //普通订单有价格
var hasPrice2 = false; //预约订单有价格
...
function totalAmt() {
	...
	if(hasPrice1 && hasPrice2){
		showMessage("预约下单不允许存在有价格的明细,请将有价格的物资重新加入购物车!");
		return false;
	}
	
	if(hasPrice1 && !hasPrice2){//这里统统都是有价格的
		$("#totalAmt").html(parseFloat(totalAmt).toFixed(2));
	}
    
	if(!hasPrice1 && hasPrice2){//预约下单,这里统统都是没有单价的
		$("#totalAmt").html("未知");
		$("#yuan").hide();
		return false;
	}
...
function getJsonString() {
    ...
    $("ul .current").each(function() {
        ...
        var checkedPm = $("#pm_" + id).val();
        var checkedDepid =$("#depid_" + id).val();
		...
		json.checkedPm = checkedPm;
		json.checkedDepid = checkedDepid;
...
function toConfirmOrder(){
    ...
    // confirmMsg("我已确认此订单信息准确无误，确定提交订单？", function() {
        ...
        var dataMap = {};        
        dataMap.jsonstring = getJsonString();
        var json = dataMap.jsonstring;
        // 先修改购物车信息
        $.ajaxjsonp(url, dataMap, function(data) {
            sessionStorage.setItem("shoppingCartData",json);
            var shoppingCartRzFlag = $("#rzFlag .current").attr("val");
            sessionStorage.setItem("shoppingCartRzFlag",shoppingCartRzFlag);
            var customerId =$("#customerId").val();
            var urlStr = "";
            var couponType =$("#couponType").val();
            if(!isNull(couponType)){
                urlStr += "couponType="+couponType;
                if(!isNull($("#activityId").val())){
                    urlStr+="&activityId="+$("#activityId").val();
                }
                if(!isNull($("#couponCode").val())){
                    urlStr+="&couponCode="+$("#couponCode").val();
                }if(!isNull($("#flag01").val())){
                    urlStr+="&flag01="+$("#flag01").val();
                }if(!isNull($("#wzid").val())){
                    urlStr+="&wzid="+$("#wzid").val();
                }if(!isNull($("#depid").val())){
                    urlStr+="&depid="+$("#depid").val();
                }if(!isNull($("#salesType").val())){
                    urlStr+="&salesType="+$("#salesType").val();
                }

            }
            if(customerId!=null && customerId!=''){
                openPage("订单结算", "../public/confirmOrder.html?customerId="+customerId+"&"+urlStr, "1");
            }else{
                openPage("订单结算", "../public/confirmOrder.html?"+urlStr, "1");
            }
        },false,function(){
            setTimeout(timeRefresh,500);
        });
...
function changrSl1Sub(pkid){
    var sl1 = $("#sl01_"+pkid).val();
    $("#sl01_"+pkid).val(--sl1);
    changeSl1(pkid);
}
function changrSl1Add(pkid){
    var sl1 = $("#sl01_"+pkid).val();
    $("#sl01_"+pkid).val(++sl1);
    changeSl1(pkid);
}
function checkAll(){
    if($("#allCheckicon ").hasClass("current")){
        $(".checkicon ").removeClass("current");
    }else{
        $(".checkicon ").addClass("current");
    }
...
function checkXbjz(pkid){
    $(".xbjz").removeClass('curr');
    if(!$("#"+pkid).hasClass('curr')){
        $("#"+pkid).addClass('curr');
    }
    // 计算合计数
    totalSl2();
